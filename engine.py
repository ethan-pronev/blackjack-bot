...
class BlackjackEngine():
	def __init__(self):
		self.runningCount = 0
		self.trueCount = 0
		self.remainingCards = 208 # just the number at the bottom (not including XX or no message errors)
		self.seenCards = 0
		
		self.balance = 0
		self.bettingUnits = 1000 # how many units to divide our bankroll by
		self.betSpread = [0,1,4,7,10,12,14,17,20,25] # how many betting units for each true count (array idx)
	...
	def updateCount(self, hands, remaining):
		if remaining < self.remainingCards: # normal case
			...
			self.trueCount = self.runningCount / (max(208 - self.seenCards, 1) / 52)
			...
			includedCards = dealtOrder[-(208-self.remainingCards):] # suffix of list we want to include (this should also work properly in the case Pancake bot misses a hand ie. 208-self.remainingCards > len(dealtOrder))
	...
	def calculateBetAmount(self):
		if self.trueCount >= 1 and not self.warmingUp:
			...
			# safeguard to try and prevent losing everything
			if self.balance < 200:
				amount = 2
